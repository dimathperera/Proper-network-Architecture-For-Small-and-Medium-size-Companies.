# Proper-network-Architecture-For-Small-and-Medium-size-Companies.
it is a collection of practical IT architecture Principals and best practices design for small and medium size companies

# Secure Network Architecture Framework for SMEs in Sri Lanka

**HND Cybersecurity & Network Architecture Final Year Project**

**Candidate:** 
**Standards:** ISO 27001 & SFIA Skills Framework  
**Repository:** [GitHub Link]

## 📋 Project Overview

This project develops a localized "Security-by-Design" framework 
for Sri Lankan startups and SMEs, providing standardized, 
affordable network architecture aligned with international standards.

## 📚 Documentation

All project documentation is organized in the `/docs` folder:

- **[Project Brief](docs/PROJECT_BRIEF.md)** - Complete project overview
- **[Purpose & Objectives](docs/01-PURPOSE_OBJECTIVES.md)** - What we're achieving
- **[Problem & Opportunity](docs/02-PROBLEM_OPPORTUNITY.md)** - Market analysis
- **[Solution Overview](docs/03-SOLUTION_OVERVIEW.md)** - Technical approach
- **[Academic Alignment](docs/04-ACADEMIC_ALIGNMENT.md)** - HND integration
- **[Deliverables](docs/05-SCOPE_DELIVERABLES.md)** - 18 key outputs
- **[Timeline](docs/07-TIMELINE.md)** - 8-month project plan
- **[Risk Assessment](docs/09-RISK_ASSESSMENT.md)** - Risk management
- **[Success Criteria](docs/10-SUCCESS_CRITERIA.md)** - How we measure success


- 


## 🚀 Getting Started

1. Read the [Project Brief](docs/PROJECT_BRIEF.md)
2. Review [Technical Architecture](technical/NETWORK_ARCHITECTURE.md)
3. Check [Implementation Timeline](docs/07-TIMELINE.md)
4. See [Deliverables](docs/05-SCOPE_DELIVERABLES.md)

## 👥 Project Team

**Lead:**  
**Program:** HND Cybersecurity & Network Architecture  
**Institution:** [Your Institution]  

## 📄 License

MIT License - See [LICENSE](LICENSE) file

## 🤝 Contributing

This is an academic project. For feedback or contributions:

1. Create an Issue with your suggestion
2. Fork the repository for your own version
3. Submit pull requests for improvements

---

**Last Updated:** March 2026  
**Status:** Active Development  
**Next Milestone:** Phase 1 Completion (Month 2)
